...
interface AnamnesisData {
  ...
  medical_conditions?: string; // Ex: hipertensão, diabetes, asma
  surgeries?: string; // Ex: apendicite, joelho, ombro
  allergies?: string; // Ex: medicamentos, alimentos
  current_pain_level?: number; // 0-10
  pain_locations?: string; // Ex: joelho esquerdo, lombar
  previous_injuries?: string; // Ex: distensão muscular, fratura
  
  // Seção 3: Medicamentos e Suplementos
  current_medications?: string; // Ex: anti-hipertensivo, insulina
  supplement_use?: string; // Ex: whey protein, creatina, multivitamínico
  
  // Seção 4: Histórico de Treino
  training_experience?: string; // iniciante, intermediario, avancado
  current_fitness_level?: string; // sedentario, iniciante, etc.
  previous_diet_experience?: string; // Ex: low carb, cetogênica
  
  // Seção 5: Estilo de Vida
  occupation?: string;
  physical_activity_work?: string; // sedentario, leve, etc.
  sleep_quality?: string; // excelente, boa, regular, ruim
  average_sleep_hours?: number;
  stress_level?: string; // baixo, moderado, alto
  smoking_status?: string; // nunca, ex_fumante, fumante_atual
  alcohol_consumption?: string; // nenhum, ocasional, etc.
  diet_type?: string; // omnívoro, vegetariano, etc.
  
  // Seção 6: Objetivos e Barreiras
  main_goal?: string; // Objetivo principal detalhado
  specific_fitness_goals?: string; // Ex: correr 5km, bench press 100kg
  timeline_for_goals?: string; // Ex: 3 meses, 6 meses
  training_frequency?: string; // 1-2, 3-4, 5+
  preferred_training_times?: string; // Manhã, tarde, noite
  barriers_to_training?: string; // Ex: falta de tempo, dor nas costas
}
...
const AnamnesisForm = ({ student, open, onOpenChange }: AnamnesisFormProps) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "Sucesso!",
        description: "Anamnese salva com sucesso. Agora você tem um perfil completo para criar treinos personalizados."
      });
      onOpenChange(false);
    } catch (error) {
        ...
        <div className="overflow-y-auto max-h-[70vh] space-y-6 p-1">
          {loading ? (
            ...
            <>
              ...
              {/* Seção 5: Estilo de Vida e Hábitos */}
