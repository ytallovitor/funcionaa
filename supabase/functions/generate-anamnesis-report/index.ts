...
serve(async (req) => {
  ...
  try {
    ...
    if (anamnesis.current_pain_level && anamnesis.current_pain_level > 3) {
      report.challenges_risks.push(`Nível de dor atual (${anamnesis.current_pain_level}/10) em ${anamnesis.pain_locations || 'área não especificada'}.`);
    ...
    if (anamnesis.barriers_to_training && anamnesis.barriers_to_training.toLowerCase() !== 'nenhuma') {
      report.challenges_risks.push(`Barreiras ao treino: ${anamnesis.barriers_to_training}.`);
